...
# Display banner
    ...
    parser.add_argument("--phone",
                        action="store", dest="phone_number",
                        help="Look up information about a phone number"
                        )
    parser.add_argument("usernames", nargs="*", metavar="USERNAMES",
                        action="store",
    ...
    if args.output is not None and len(args.usernames) != 1:
    ...
    if args.phone_number:
        lookup_phone(args.phone_number)
        return
...
def lookup_phone(phone):
    ...
    if response.get("valid"):
        print(f"Phone: {response.get('international_format')}")
        print(f"Country: {response.get('country_name')} ({response.get('country_code')})")
        print(f"Location: {response.get('location')}")
        print(f"Carrier: {response.get('carrier')}")
        print(f"Line Type: {response.get('line_type')}")
    else:
